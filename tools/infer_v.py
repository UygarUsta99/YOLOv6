...
def run(weights=osp.join(ROOT, 'yolov6s.pt'),
    ...
    if (source.split("/")[-1].split(".")[-1] in ["mp4","h264"]) or source == '0':
        ...
        while 1:
            _,frame = cap.read()
            inferer.infer(conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf,frame)
